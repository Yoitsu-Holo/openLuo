...
public abstract class LlmClientBase : ILlmClient
{
    ...
    protected readonly LlmRouteConfig Config;
    ...
    protected LlmClientBase(LlmRouteConfig config)
    {
        Config = config.Clone();
    ...
    protected abstract Task<LlmChatResponse> CompleteCoreAsync(
    ...
    protected LlmOptions MergeOptions(LlmOptions? options) => new()
    {
        ...
        Tools = options?.Tools,
        RequiredCapabilities = options?.RequiredCapabilities?.Clone() ?? new(),
    ...
    protected IReadOnlyList<LocalChatMessage> SanitizeMessages(IEnumerable<LocalChatMessage> messages) =>
        messages.Select(m =>
        {
            var sanitized = m.Role == ChatMessageRole.User
                ...
                : m.Content;
            return new LocalChatMessage(m.Role, sanitized) { Blocks = m.Blocks, ToolCalls = m.ToolCalls, ToolCallId = m.ToolCallId };
        }).ToList();
        ...
        for (var attempt = 0; attempt <= retries; attempt++)
        {
            try
            {
                ...
                if (sw.ElapsedMilliseconds > 10000)
                    Warn("llm", $"{operationName} completed: duration={sw.ElapsedMilliseconds}ms");
                else
                    Info("llm", $"{operationName} completed: duration={sw.ElapsedMilliseconds}ms");
                return result;
            }
            catch (OperationCanceledException) when (!ct.IsCancellationRequested && attempt < retries)
            {
                ...
                Warn("llm", $"{operationName} timeout, retrying in {delay}ms (attempt {attempt + 1}/{retries})");
                await Task.Delay(delay, ct);
            }
            catch (HttpRequestException ex) when (attempt < retries)
            {
                var delay = GetBackoffDelayMs(attempt, delayBase);
                Warn("llm", $"{operationName} failed, retrying in {delay}ms (attempt {attempt + 1}/{retries}): {ex.Message}");
                await Task.Delay(delay, ct);
            }
            catch (Exception ex)
            {
                sw.Stop();
                Error("llm", $"{operationName} failed after {sw.ElapsedMilliseconds}ms: {ex.Message}");
                throw;
            }
        }
