...
public sealed class OllamaLlmClient : LlmClientBase
{
    private static readonly JsonSerializerOptions HttpJsonOptions = new(JsonSerializerDefaults.Web);

    public OllamaLlmClient(LlmRouteConfig config) : base(config)
    {
    }
    ...
    protected override Task<LlmChatResponse> CompleteCoreAsync(IReadOnlyList<LocalChatMessage> messages, LlmOptions options, CancellationToken ct)
    {
        Info("llm", $"LLM custom request started: provider={Config.Provider}, model={Config.Model}, temp={options.Temperature}, messages={messages.Count}");
        Debug("llm", $"LLM custom prompt: {JsonSerializer.Serialize(messages.Select(m => new { m.Role, m.Content }), LogOptions)}");
        ...
        return ExecuteWithRetryAsync("LLM custom request", async linked =>
        {
            ...
            Debug("llm", $"LLM custom response (raw): {text}");
            return new LlmChatResponse { Content = text };
        }, ct);
    }
    ...
    {
        Info("llm", $"LLM custom streaming request started: provider={Config.Provider}, model={Config.Model}, temp={options.Temperature}, messages={messages.Count}");
        Debug("llm", $"LLM custom prompt: {JsonSerializer.Serialize(messages.Select(m => new { m.Role, m.Content }), LogOptions)}");
        ...
        return ExecuteWithRetryAsync("LLM custom streaming request", async linked =>
        {
            ...
            Debug("llm", $"LLM custom streaming response (raw): {fullResponse}");
            return fullResponse;
        }, ct);
    }
    ...
    private static bool IsQwenModel(LlmRouteConfig config) =>
