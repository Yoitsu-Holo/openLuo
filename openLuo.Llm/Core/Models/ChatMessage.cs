...
public class ChatMessage
{
    ...
    /// <summary>
    /// Tool calls requested by the model (assistant role messages). When non-empty, the
    /// message is an assistant turn that asked the host to execute tools.
    /// </summary>
    public IReadOnlyList<LlmToolCall>? ToolCalls { get; init; }

    /// <summary>
    /// Id of the tool call this message is a result for (role:tool messages).
    /// </summary>
    public string? ToolCallId { get; init; }

    /// <summary>
    /// Human-readable content summary for debug logging.
    /// When Blocks are present, each block contributes a compact string (images show sampled base64).
    /// </summary>
    public string DebugContent
    {
        get
        {
            ...
            return string.IsNullOrWhiteSpace(joined) ? Content : joined;
        }
    }
