using openLuo.Modules.Executor.Core.Interfaces;
using openLuo.Modules.Executor.Infrastructure;
using openLuo.Core.Interfaces;
using openLuo.Modules.Llm.Core.Interfaces;

namespace openLuo.Modules.Executor.Application.GiftIntent;

public sealed class GiftIntentExecutor : LlmStructuredExecutor<GiftIntentInput, GiftIntentOutput>
{
    public GiftIntentExecutor(
        ILlmClient llmClient,
        IExecutorPromptBuilder<GiftIntentInput> promptBuilder,
        IStructuredOutputParser outputParser,
        IGameLogger? logger = null)
        : base(llmClient, promptBuilder, outputParser, logger)
    {
    }

    public override string Name => "gift_intent";
}
