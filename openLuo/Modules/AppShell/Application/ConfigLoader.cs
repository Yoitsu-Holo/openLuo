...
public static class ConfigLoader
{
    private static readonly JsonSerializerOptions JsonOptions = new()
    {
        ...
        // JSONC 手写配置文件允许尾随逗号（惯例），避免最后一个字段后的逗号导致整个文件静默失败
    ...
    public static AppConfig LoadDirectory(string configDir, ILogger? logger = null)
    {
        ...
        logger?.LogInformation("Loading config directory: {ConfigDir}", configDir);
        ...
        foreach (var runtimePath in GetRuntimeConfigFiles(configDir))
        {
            ...
            {
                logger?.LogInformation("Loaded runtime config: {FileName}", Path.GetFileName(runtimePath));
                merged = Merge(merged, partial);
            }
        }

        logger?.LogInformation("Config load complete");
        merged.Normalize();
        return merged;
    }
    ...
    private static AppConfig? LoadPartial(string path, ILogger? logger = null)
    {
        try
        {
            ...
            {
                logger?.LogInformation("Skipped empty config file: {FileName}", Path.GetFileName(path));
                return null;
            }
        ...
        {
            logger?.LogError(ex, "Failed to load config file: {FileName}", Path.GetFileName(path));
            return null;
        }
    }
