...
public sealed class RuntimeConfigCenter : IRuntimeConfigCenter, IDisposable
{
    ...
    private void OnConfigFileChanged(object? sender, FileSystemEventArgs e)
    {
        ...
        if (name is null || name.StartsWith('.') || name.EndsWith('~'))
            return;
        lock (_reloadGate)
    ...
    private void ReloadFromDisk()
    {
        try
        {
            ...
            _snapshot = next;
            openLuo.Infrastructure.Config.Config.UpdateSnapshot(next);

            if (deferredPaths.Count > 0)
            {
                _logger?.LogWarning(
                    "Config changes requiring restart detected; hot-update ignored: {DeferredPaths}",
                    string.Join(", ", deferredPaths));
            }
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "Config hot-reload failed, keeping current snapshot");
        }
    }
