...
def log(level, msg):
    call_host("host/log", {"pluginId": PLUGIN_ID, "level": level, "msg": msg})
...
def handle_help(args):
    cmds = call_host("host/commands/list")
    cmds = cmds or []
    if not cmds:
...
def handle_give(args):
    ...
    result = call_host("plugin/gift/execute", {"itemRef": item_ref})
...
def handle_debug(args):
    ...
    state = call_host("core/session/get")
    char = call_host("core/character/get")
...
def _clear_timeline_events_for_status(status, limit=200, max_rounds=1000):
    ...
    while rounds < max_rounds:
        rounds += 1
        query_res = call_host(
            "plugin/timeline/query", {"status": status, "limit": limit, "offset": 0}
        )
        ...
        for item in items:
            ...
            if not event_id:
                failed += 1
                continue
            cancel_res = call_host("plugin/timeline/cancel", {"eventId": event_id})
...
def handle_time_mode(args):
    ...
    snapshot = call_host("core/time/get") or {}
    if not args:
    ...
    apply_res = call_host("core/state/apply", {"mutations": mutations})
    ...
    after = call_host("core/time/get") or {}
    cleanup_line = ""
