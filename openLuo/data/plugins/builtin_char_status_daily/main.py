...
def call_host(method, params=None):
    global _req_id
    _req_id += 1
    req = {"jsonrpc": "2.0", "id": f"d{_req_id}", "method": method, "params": params or {}}
...
def onStartup(args):
    ...
    for res in state_defs:
        call_host("core/state/register", {"def": res})
    return {"success": True}
...
def onPromptContext(args):
    ...
    stress = int(float(snapshot.get("stress", 30)))
    fatigue = int(float(snapshot.get("fatigue", 20)))

    char_config = load_character_config(character_id) if character_id else None
    fragments = []

    # mood prompt
    mood_prompt = get_prompt_for_value(char_config, "mood", mood)
    if mood_prompt:
        fragments.append({"phase": args.get("phase", "chat-evaluate"), "priority": 80, "text": mood_prompt})

    # stress prompt (只在高压时提示)
    if stress >= 70:
        stress_prompt = get_prompt_for_value(char_config, "stress", stress)
        if stress_prompt:
            fragments.append({"phase": args.get("phase", "chat-evaluate"), "priority": 75, "text": stress_prompt})

    # fatigue prompt (只在重度疲劳时提示)
    if fatigue >= 70:
        fatigue_prompt = get_prompt_for_value(char_config, "fatigue", fatigue)
        if fatigue_prompt:
            fragments.append({"phase": args.get("phase", "chat-evaluate"), "priority": 70, "text": fatigue_prompt})
        ...
        "statusItems": [
            {
                ...
                "label": "心情",
                "type": "text",
                "value": snapshot.get("mood", "Neutral"),
                "group": "emotion",
                "order": 150,
            },
            {
...
TOOLS_LIST = [
    {"name": "onStartup", "description": "Register daily states", "category": "hook"},
    {"name": "onPromptContext", "description": "Provide daily status context", "category": "hook"},
    {"name": "onStatusQuery", "description": "Provide daily status display", "category": "hook"},
]
    ...
    if method == "initialize":
        return {
            ...
            "result": {
                ...
                "serverInfo": {"name": "builtin_char_status_daily", "version": "1.0.0"},
            },
        }
