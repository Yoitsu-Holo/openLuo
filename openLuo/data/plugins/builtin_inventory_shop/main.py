...
def handle_shop(args):
    categories_result = call_host("plugin/shop/categories") or {}
    categories = (
...
def handle_buy(args):
    ...
    result = call_host(
        "plugin/shop/buy", {"categoryIndex": cat_index, "itemIndex": item_index}
    )
...
def handle_inventory(args):
    inv = call_host("plugin/inventory/get")
    all_items = call_host("plugin/items/list")
    if not inv:
