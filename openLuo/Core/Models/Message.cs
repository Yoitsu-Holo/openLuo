...
public sealed class Message
{
    ...
    public static Message FromText(string text, string speakerRole = "assistant", string? speakerId = null)
    {
        ...
        return new Message
        {
            ...
            [
                ...
                {
                    Kind = BlockKind.Text,
                    Source = ResolveSource(speakerRole),
                    Text = text
                }
            ]
        };
    ...
    private static BlockSource ResolveSource(string speakerRole) =>
        speakerRole.Equals("user", StringComparison.OrdinalIgnoreCase) ||
        speakerRole.Equals("inbound", StringComparison.OrdinalIgnoreCase)
            ? BlockSource.User
            : BlockSource.Agent;
