...
**openLuo** 是一个开源的”可扩展 AI 角色引擎”，目标是构建可持续演进的洛天依式虚拟角色交互体验底座。项目以插件化架构为核心，将 LLM 推理、角色扮演、状态管理、记忆检索、多角色协作等能力解耦为独立模块，支持 CLI / TUI 双入口运行，并面向 GUI、语音、Live2D、多模态等交互通道持续演进。
...
|------|----------|
| 语言 / 运行时 | C# 13 / .NET 10 |
| AI / LLM 抽象 | `Microsoft.Extensions.AI`（OpenAI provider） |
| 向量检索 | SQLite + Dapper + `sqlite-vec` |
| 图形渲染 | SkiaSharp |
| 终端 UI | Terminal.Gui（CLI / TUI 双入口） |
| 插件 / 扩展 | `extension.jsonc` + 独立程序集；官方 MCP / A2A 适配器接入远程能力 |
| 测试 | xUnit / NSubstitute；内核、扩展、CLI、E2E 分工程 |

---

# openLuo 架构：通用 Agent 内核 + 领域扩展

| 层级 | 工程 | 职责 |
|------|------|------|
| 基础 | `openLuo.Foundation` / `openLuo.Llm` / `openLuo.Memory` | 基础设施、LLM、Embedding、记忆端口与实现 |
| 内核契约 | `openLuo.Capabilities` / `openLuo.AgentContext` | 能力目录、决策循环、调度、状态事务、上下文快照、输出队列 |
| 桥接 | `openLuo.Capabilities.Llm` / `.Mcp` / `.A2A` | LLM 原生 tool calls、MCP、Agent2Agent 远程能力 |
| 扩展宿主 | `openLuo.Extensions.Host` | manifest、依赖拓扑、程序集加载、命名空间化、失败隔离 |
| 领域扩展 | `extensions/{memory,companion,world,party,media}` | 记忆、人格、世界状态、多角色、媒体；每个扩展自带 manifest |
| 平台 | `openLuo.Cli` / `openLuo.Tui` / `openLuo.Gui` / `openLuo.Qqbot` | 输入解析、输出渲染与平台传输 |
| 宿主 | `openLuo` | 组合根、配置、SQLite 对话存储、入口分发 |
| Demo | `openLuo.playgraound`（程序集名 `openLuo.Playground`） | 新内核能力循环最小可运行演示 |

领域扩展目录以 `.disable` 结尾时完全跳过。扩展注册的 `canonicalId` 自动命名空间化为 `<extension-id>:<local-id>`；`core:` 保留给内核。

旧 `openLuo/Modules` 业务链路仍由现有 QQ/TUI 入口使用；新内核与扩展链路已独立编译、测试并提供 CLI/Playground 验证入口。

## 4. 能力与协议适配

- **MCP**：`openLuo.Capabilities.Mcp` 使用官方 `ModelContextProtocol` 2.1.0，支持 stdio/http/streamable-http，连接失败结构化降级。
- **A2A**：`openLuo.Capabilities.A2A` 使用 `A2A` 1.0.0-preview2，通过 Agent Card 发现 skills 并映射为 `RemoteAgent` 能力。
