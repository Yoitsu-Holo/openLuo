...
public class DefaultCapabilityDecisionLoopTests
{
    ...
    public async Task Run_ModelReturnsFinalText_EndsWithFinalReply()
    {
        ...
        model.DecideAsync(Arg.Any<CapabilityDecisionContext>(), Arg.Any<CancellationToken>())
            .Returns(new CapabilityDecision { Messages = [new FlowItem { Mode = FlowMode.Respond, Kind = ReplyItemKind.Text, Payload = "你好，今天过得怎么样？" }] });
    ...
    public async Task Run_ToolCallThenFinalText_ExecutesToolAndContinues()
    {
        ...
        model.DecideAsync(Arg.Any<CapabilityDecisionContext>(), Arg.Any<CancellationToken>())
            .Returns(
                ...
                new CapabilityDecision { Messages = [new FlowItem { Mode = FlowMode.Respond, Kind = ReplyItemKind.Text, Payload = "查到了" }] });
    ...
    public async Task Run_TerminalCapability_EndsAfterExecution()
    {
        ...
        model.DecideAsync(Arg.Any<CapabilityDecisionContext>(), Arg.Any<CancellationToken>())
            .Returns(
                ...
                new CapabilityDecision { Messages = [new FlowItem { Mode = FlowMode.Respond, Kind = ReplyItemKind.Text, Payload = "不应到达" }] });
    ...
    public async Task Run_RejectedBatch_FeedsBackReasonAndContinues()
    {
        ...
        model.DecideAsync(Arg.Any<CapabilityDecisionContext>(), Arg.Any<CancellationToken>())
            .Returns(
                ...
                new CapabilityDecision { Messages = [new FlowItem { Mode = FlowMode.Respond, Kind = ReplyItemKind.Text, Payload = "已纠正" }] });
    ...
    [Fact]
    public async Task Run_ToolCallWithInqueueText_EnqueuesInterimAndContinues()
    {
        var model = Substitute.For<ICapabilityDecisionModel>();
        model.DecideAsync(Arg.Any<CapabilityDecisionContext>(), Arg.Any<CancellationToken>())
            .Returns(
                new CapabilityDecision
                {
                    Messages = [new FlowItem { Mode = FlowMode.Inqueue, Kind = ReplyItemKind.Text, Payload = "你是想让我发张图来看看吗？" }],
                    Calls =
                    [
                        new CapabilityCall { InvocationId = "inv-1", IdempotencyKey = "k1", CanonicalId = "test:lookup", ParentDecisionId = "d1" }
                    ]
                },
                new CapabilityDecision { Messages = [new FlowItem { Mode = FlowMode.Respond, Kind = ReplyItemKind.Text, Payload = "给你发一张" }] });
        ...
        Assert.True(result.Success);
        Assert.Equal("给你发一张", result.FinalText);
        Assert.Equal(2, result.DecisionsUsed);
        // 中途文本已入队（即时推送通道），且不混入 TurnResult.Outputs（避免平台双发）
        OutputItem? enqueued = null;
        await foreach (var item in queue.ReadAsync(CancellationToken.None))
        {
            enqueued = item;
            break; // ReadAsync 是长生命周期流（等待新条目），只验证第一条
        }
        Assert.NotNull(enqueued);
        Assert.Equal("你是想让我发张图来看看吗？", Convert.ToString(enqueued!.Payload));
        Assert.Equal("s1", enqueued.ConversationId);
