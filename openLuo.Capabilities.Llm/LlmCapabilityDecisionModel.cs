        ...
        CancellationToken ct = default)
    {
        ...
        foreach (var message in context.Conversation)
        {
            messages.Add(message.Role switch
            {
                ...
                _ => new ChatMessage(ChatMessageRole.User, message.Content)
            });
        }

        // 当前用户输入
        if (!string.IsNullOrWhiteSpace(context.UserInput))
            messages.Add(new ChatMessage(ChatMessageRole.User, context.UserInput));
        ...
        {
            // D2：无 tool_call 的非空文本 = 最终回复
            return new CapabilityDecision { FinalText = response.Content };
        }

        // D17：伴随 tool_call 的文本为内部过程文本，不进入公共输出
        ...
        {
            Calls = calls,
            InternalText = string.IsNullOrWhiteSpace(response.Content) ? null : response.Content
        };
    }
