...
public sealed class DefaultCapabilityDecisionLoop : ICapabilityDecisionLoop
{
    ...
    public async Task<DecisionLoopResult> RunAsync(DecisionLoopRequest request, CancellationToken ct = default)
    {
        ...
        for (var decision = 1; decision <= budgets.MaxDecisions; decision++)
        {
            ...
            // D2：Respond 消息（无 tool_call 文本）= 最终回复
            var respondText = decisionResult.Messages
                .Where(m => m.Mode == FlowMode.Respond)
                .Select(m => Convert.ToString(m.Payload))
                .FirstOrDefault(m => !string.IsNullOrWhiteSpace(m));
            if (decisionResult.Calls.Count == 0 && !string.IsNullOrWhiteSpace(respondText))
            {
                steps.Add(new AgentToolUseStep
                {
                    ...
                    Name = "final_text",
                    Success = true,
                    Summary = Truncate(respondText, 120),
                    DurationMs = sw.ElapsedMilliseconds
                });
                // FinalText 传完整文本（steps.Summary 是截断版，不能作为回复内容）
                return Finish(steps, outputs, decisionsUsed, TerminationReason.FinalReply, "final reply", respondText);
            }

            // 空回复：继续一次受限决策（D2 配套）
            if (decisionResult.Calls.Count == 0
                && !decisionResult.Messages.Any(m => m.Mode == FlowMode.Inqueue))
            {
                noProgressStreak++;
            ...
            // Inqueue 消息：入输出队列（群聊中途即时反馈，不结束回合）。
