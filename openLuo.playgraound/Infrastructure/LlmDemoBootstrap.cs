...
internal static class LlmDemoBootstrap
{
    public static ILlmClient? TryCreateClient(out string? error)
    {
        ...
        if (route.Provider != LlmProvider.Ollama &&
            (string.IsNullOrWhiteSpace(route.ApiKey) ||
             string.Equals(route.ApiKey, "sk-your-api-key-here", StringComparison.Ordinal)))
        {
        ...
        error = null;
        return LlmClientFactory.Create(route);
    }
